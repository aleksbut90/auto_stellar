# Windows API SendMessage clicking implementation
# Direct Windows API calls using SendMessage (synchronous)
# This is the method used in the collection tool - faster and more responsive
...
class SendMessageClicker(BaseClicker):
    """Clicking implementation using Windows API SendMessage (synchronous)"""
    
    def __init__(self, game_window=None, get_offset_callback=None):
        """
        Initialize SendMessage clicker
    ...
    def click(self, coords, adjust_for_client_area=True):
        """Click using Windows API SendMessage - synchronous and reliable"""
        ...
        try:
            ...
            # Send mouse down and up messages directly - faster than pywinauto
    ...
    def right_click(self, coords, adjust_for_client_area=True):
        """Right-click using Windows API SendMessage"""
        if not self.game_window:
            return False
    ...
    def middle_click(self, coords, adjust_for_client_area=True):
        """Middle-click using Windows API SendMessage"""
        if not self.game_window:
            return False
        ...
        try:
            ...
            # Send middle mouse down and up messages
    ...
    def get_name(self):
        ...
        return "Windows API SendMessage (Synchronous)"